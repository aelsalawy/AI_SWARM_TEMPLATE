...
|---|------|-----|-------------------|------------|
| R1 | **Leaked key blast radius is 9 files, not 1** — the stub's `e5b1beed-[REDACTED]` IS the legacy shared `AGENT_API_KEY` (break-glass path, `index.ts:110-120`), also in `.env`, `.env.new`, `seed-alm.js`, 4 root docs, git history (`7fbe642`). Any holder impersonates any agent | High | **Worse** than chat doc R3 stated | Task 1: rotate legacy key everywhere at once (single value = single rotation), delete/scrub files, history purge if repo shared. Do regardless of this decision |
