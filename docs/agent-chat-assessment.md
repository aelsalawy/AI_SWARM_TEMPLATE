...
| C5 | Agent trigger stub | `server/routes/agent-trigger.ts` | `POST /api/agents/trigger` validates input, **spawns nothing**, and embeds a **hard-coded agent API key in source** (line 52: `e5b1beed-…`). Security bug — must be revoked and the route removed/rewritten regardless of this proposal. |
